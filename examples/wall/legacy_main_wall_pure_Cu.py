...
# Pure-Cu control run: same geometry/toolpath/laser as thinwall_graded.k,
# but the deposited wall is 100% Cu (thinwall_Cu.k - matID 1 swapped to
# Cu properties, matID 2 base plate left as TI64) - used as the other
# endpoint of a TI64-vs-Cu-vs-graded temperature comparison.
...
domain = domain_mgr(filename='thinwall_Cu.k')
...
t = 0
filename = 'results_Cu/wall_{:04d}.vtu'.format(file_num)
save_vtk(filename, 0.0)
file_num = file_num + 1

# ---- run settings ----
STOP_FRACTION = 0.15    # shortened preview depth - Cu's much smaller stable
                        # timestep made the original 0.5 depth too slow; 0.15
                        # still reaches several deposited layers
N_FRAMES = 200
...
while domain.current_sim_time < endtime - domain.dt:
    ...
    if domain.current_sim_time >= next_save_time:
        save_vtk('results_Cu/wall_{:04d}.vtu'.format(file_num), float(domain.current_sim_time))
        file_num = file_num + 1
...
print('\nDone - {} frames written to results_Cu/'.format(file_num))
