# Proves out *EXTERNAL_MESH import against the externally-imported mesh
# (sol_100.vtu): loads the
...
os.makedirs('results/external_mesh_import_check', exist_ok=True)

domain = domain_mgr(filename='external_mesh/sol_100_control.k', sort_birth=False)
...
ext = meshio.read('external_mesh/sol_100.vtu')
...
file_num = 0
save_vtk('results/external_mesh_import_check/wall_{:04d}.vtk'.format(file_num), 0.0)
file_num += 1
...
while domain.current_sim_time < STOP_TIME:
    ...
    if domain.current_sim_time >= next_save_time:
        save_vtk('results/external_mesh_import_check/wall_{:04d}.vtk'.format(file_num), float(domain.current_sim_time))
        file_num += 1
        next_save_time += save_interval
...
print('\nDone - {} frames written to results/external_mesh_import_check/'.format(file_num + 1))
