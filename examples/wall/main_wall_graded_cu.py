...
domain = domain_mgr(filename='thinwall_graded_cu.k')
...
t = 0
filename = 'results_graded_cu/wall_{:04d}.vtu'.format(file_num)
...
while domain.current_sim_time < endtime - domain.dt:
    ...
    if domain.current_sim_time >= next_save_time:
        save_vtk('results_graded_cu/wall_{:04d}.vtu'.format(file_num), float(domain.current_sim_time))
        file_num = file_num + 1
...
print('\nDone - {} frames written to results_graded_cu/'.format(file_num))
