...
# sinusoidal) on part003 - generalization check, same script structure as
...
# ---- report ----
lines = []
lines.append('10%-depth composition-mode thermal comparison: toolpath_arc_length vs global_x (part003)')
lines.append('=' * 70)
...
vtu_path = os.path.join(args.out, 'part003_composition_mode_comparison_10pct_final_frame.vtu')
...
ax.set_title('Peak temperature vs. time (10% depth, part003)')
...
ax.set_title('Mean active-build-region temperature vs. time (10% depth, part003)')
ax.legend(); ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False)
...
ax.set_title('Maximum temperature difference vs. time (10% depth, part003)')
