...
domain = domain_mgr(filename='thinwall_graded_linear.k')
...
t = 0
filename = 'results_graded_linear/wall_{:04d}.vtk'.format(file_num)
...
while domain.current_sim_time < endtime - domain.dt:
    ...
    if domain.current_sim_time >= next_save_time:
        save_vtk('results_graded_linear/wall_{:04d}.vtk'.format(file_num), float(domain.current_sim_time))
        file_num = file_num + 1
...
print('\nDone - {} frames written to results_graded_linear/'.format(file_num))
