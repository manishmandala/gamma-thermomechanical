...
def linear_mix(values, f):
    # values: list of N per-material terms (python floats, or arrays that
    # broadcast against f[..., k]). f's last axis is always the N-materials
    # axis; callers add extra leading/middle axes to f (e.g. a Gauss-point
    # broadcast axis) to match whatever shape `values` entries have.
    return sum(f[..., k] * values[k] for k in range(len(values)))


# Default per-property blend rule for *MAT_THERMAL_GRADED_TD regions - every
# property defaults to linear rule-of-mixtures but can be overridden per
# property via domain_mgr(mixing_rules=...), e.g. once real (non-linear)
# mixture data justifies it for solidus/liquidus. Mirrors gradient_material.py's
# MIXING_RULES so the same mental model applies whether blending happens at
# preprocessing time (discrete bands) or at runtime (continuous field).
...
class domain_mgr():
    def __init__(self, filename, sort_birth=True, toolpathdir='toolpath.crs', verbose=True, timestep_override=-1,
                 input_data_dir=".", mixing_rules=None):
    ...
    def load_file(self):
        ...
        #         # option *SET_NODE_LIST
                ...
                # multi-channel "mat" field) is a separate, not-yet-built step.
                    ...
                    n_mat_graded = len(mat_graded[0][1]) if mat_graded else 1
                    composition_list_element = [[0.0]*n_mat_graded for _ in range(len(elements))]
                    break
                ...
                # option *MAT_THERMAL_GRADED_TD - a continuous blend between N
                # endpoint materials, embedded directly rather than referencing
                # existing mat_thermal entries by ID. This is deliberate:
                # mat_thermal/thermal_TD entries must be indexed sequentially
                # (matID == file position + 1, see get_timestep) for a legacy
                # reason, and a graded region that never registers here
                # sidesteps that constraint entirely instead of having to satisfy it.
                # Data line: gradID, N, then 4 properties (density, solidus,
                # liquidus, latent) per material, N materials total. Followed
                # by N (Cp_file, Cond_file) line pairs, one per material.
                    ...
                    n_mat = int(text1[1])
                    props = text1[2:]
                    mat_props = [tuple(float(v) for v in props[4*k:4*k+4]) for k in range(n_mat)]
                    mat_graded.append([gradID, mat_props])
                    curves = []
                    for k in range(n_mat):
                        line = next(f)
                        Cp_k = np.loadtxt(os.path.join(self.input_data_dir, line.split()[0]))
                        line = next(f)
                        Cond_k = np.loadtxt(os.path.join(self.input_data_dir, line.split()[0]))
                        curves.append((Cp_k, Cond_k))
                    graded_TD[gradID] = curves
        ...
        with open(filename) as f:
                while True:
                    ...
                    if line.split()[0] == '*ELEMENT_COMPOSITION':
                        line = next(f)   # gradID header line (which *MAT_THERMAL_GRADED_TD block this table feeds - not needed for a single-region file, kept for future multi-region support)
                        while True:
                            line = next(f)
                            if line[0] == '*':
                                break
                            if line[0] == '$':
                                continue
                            text = line.split()
                            eid = int(float(text[-1]))   # last column is always the element id
                            fracs = [float(v) for v in text[:-1]]   # one fraction per material, N columns
                            composition_list_element[eid-element_base] = fracs
                    if line.split()[0] == '*END':
                        break
    ...
    def get_timestep(self):
        ...
        for gradID, curves in self.graded_TD.items():
            ...
            _, mat_props = rec
            rhos = [p[0] for p in mat_props]
            mat = self.element_mat == gradID
            if mat.sum() == 0:
                continue
            l = ele_length[mat]
            f = self.element_composition[mat]   # (n_masked, N)
            mix = self.mixing_rules
            min_cps = [Cp_k[:,1].min() for Cp_k, Cond_k in curves]
            max_conds = [Cond_k[:,1].max() for Cp_k, Cond_k in curves]
            density = mix['density'](rhos, f)
            min_Cp = mix['cp'](min_cps, f)
            max_Cond = mix['cond'](max_conds, f)
...
class heat_solve_mgr():
    ...
    def update_cp_cond(self):
        ...
        ##### graded (continuous composition) regions: blend the N endpoint
        ##### materials' curves independently at each element's own current
        ##### temperature, then combine via the per-property mixing rule - a
        ##### true per-element/per-Gauss-point evaluation, not a pre-blended
        ##### curve on a shared grid (that's what gradient_material.py's
        ##### discrete-band approach does at preprocessing time instead).
        mix = domain.mixing_rules
        for gradID, curves in domain.graded_TD.items():
            rec = next(r for r in domain.mat_graded if r[0] == gradID)
            _, mat_props = rec
            rhos  = [p[0] for p in mat_props]
            sols  = [p[1] for p in mat_props]
            liqs  = [p[2] for p in mat_props]
            lats  = [p[3] for p in mat_props]
            mat = domain.element_mat == gradID
            active_mat = domain.active_elements*mat
            thetaIp = temperature_ip[active_mat]
            f = domain.element_composition[active_mat]
            # (n_masked,1,N): every mix() call below uses this same shape, so
            # each result comes out (n_masked,1) - matching thetaIp's
            # (n_masked,8) via broadcast, same as the old single-fraction f.
            f_ip = f[:,cp.newaxis,:]

            density = mix['density'](rhos, f_ip)
            solidus = mix['solidus'](sols, f_ip)
            liquidus = mix['liquidus'](liqs, f_ip)
            latent = mix['latent'](lats, f_ip)/(liquidus-solidus)

            self.density_Cp_Ip[active_mat] += density*latent*(thetaIp>solidus)*(thetaIp<liquidus)

            Cp_ip_k = [cp.interp(thetaIp, cp.asarray(Cp_k[:,0]), cp.asarray(Cp_k[:,1])) for Cp_k, Cond_k in curves]
            self.density_Cp_Ip[active_mat] += density*mix['cp'](Cp_ip_k, f_ip)

            Cond_ip_k = [cp.interp(thetaIp, cp.asarray(Cond_k[:,0]), cp.asarray(Cond_k[:,1])) for Cp_k, Cond_k in curves]
            self.Cond_Ip[active_mat] += mix['cond'](Cond_ip_k, f_ip)
